...
import { CargoStats, DepositCargoToFleetInput, Fleet, FleetStateData, IdleToLoadingBayInput, LoadingBayToIdleInput, MovementStats, PlanetType, Sector, ShipStats, StarbaseCreateCargoPodInput, StarbasePlayer, StartMiningAsteroidInput, StartSubwarpInput, StopMiningAsteroidInput, WarpToCoordinateInput, WithdrawCargoFromFleetInput } from "@staratlas/sage";
...
export class SageFleet {
    ...
    async ixLoadCargo(resourceName: ResourceName, cargoPodType: CargoPodType, amount: BN) {
      ...
      const cargoPod = await this.getCurrentCargoPodDataByType(cargoPodType);
    ...
    async ixStopMining() {
      ...
      const cargoPod = await this.getCurrentCargoPodDataByType(CargoPodType.CargoHold);
      if (cargoPod.type !== "Success") return cargoPod;

      const mintAta = cargoPod.data.loadedResources.get(miningMint.toBase58());
      const foodAta = cargoPod.data.loadedResources.get(miningMint.toBase58());
      if (!foodAta) return { type: "FleetCargoPodTokenAccountNotFound" as const };

      const ammoBank = await this.getCurrentCargoPodDataByType(CargoPodType.AmmoBank);
      if (ammoBank.type !== "Success") return ammoBank;

      const ammoAta = ammoBank.data.loadedResources.get(this.getSageGame().getResourcesMint().Food.toBase58());
      if (!ammoAta) return { type: "FleetCargoPodTokenAccountNotFound" as const };

      const fuelTank = await this.getCurrentCargoPodDataByType(CargoPodType.FuelTank);
      if (fuelTank.type !== "Success") return fuelTank;

      const fuelAta = fuelTank.data.loadedResources.get(this.getSageGame().getResourcesMint().Fuel.toBase58());
      if (!fuelAta) return { type: "FleetCargoPodTokenAccountNotFound" as const };
